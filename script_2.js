window.addEventListener('load', () => {
    ...
    function initSlider() {
        ...
        // Clear existing clones if function is re-run (though for simple resize we reload)
        // Ideally we would clean up properly, but for this snippet we assume fresh load
        // To be safe on re-runs in a more complex app, we'd clear innerHTML here if it wasn't the first run.
        
        // slider.innerHTML = '';
        // originalItems.forEach(item => slider.appendChild(item));

        // NOTE: In this simplified script, we assume items are already in place or cloned once.
        // If we clone repeatedly on resize without clearing, we get too many items.
        // The safest approach for this structure is to handle resizing via CSS and basic refresh.
        
        // For the sake of this specific code block which clones 4 times:
        // We will only clone if we haven't already cloned (check for a class or attribute)
        ...
        // We need to wait a tick for styles to apply if we just cloned
        const itemStyle = getComputedStyle(originalItems[0]);
        // Use getBoundingClientRect for more accurate sub-pixel rendering
        const itemRect = originalItems[0].getBoundingClientRect();
        const itemWidth = itemRect.width + parseFloat(itemStyle.marginRight);
    ...
    window.addEventListener('resize', () => {
        ...
        resizeTimeout = setTimeout(() => {
            ScrollTrigger.refresh();
            // Optional: Re-initialize slider if needed, but CSS scaling usually handles basic resize
        }, 200);
    });
